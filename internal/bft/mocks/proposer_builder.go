...
func (_m *ProposerBuilder) NewProposer(leader uint64, proposalSequence uint64, viewNum uint64, decisionsInView uint64, quorumSize int) (bft.Proposer, bft.Phase) {
	...
	var r0 bft.Proposer
	var r1 bft.Phase
	if rf, ok := ret.Get(0).(func(uint64, uint64, uint64, uint64, int) (bft.Proposer, bft.Phase)); ok {
		return rf(leader, proposalSequence, viewNum, decisionsInView, quorumSize)
	}
...
type mockConstructorTestingTNewProposerBuilder interface {
	mock.TestingT
	Cleanup(func())
}

// NewProposerBuilder creates a new instance of ProposerBuilder. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewProposerBuilder(t mockConstructorTestingTNewProposerBuilder) *ProposerBuilder {
	mock := &ProposerBuilder{}
	mock.Mock.Test(t)
