...
func (_m *RequestPool) NextRequests(maxCount int, maxSizeBytes uint64, check bool) ([][]byte, bool) {
	...
	var r0 [][]byte
	var r1 bool
	if rf, ok := ret.Get(0).(func(int, uint64, bool) ([][]byte, bool)); ok {
		return rf(maxCount, maxSizeBytes, check)
	}
...
type mockConstructorTestingTNewRequestPool interface {
	mock.TestingT
	Cleanup(func())
}

// NewRequestPool creates a new instance of RequestPool. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewRequestPool(t mockConstructorTestingTNewRequestPool) *RequestPool {
	mock := &RequestPool{}
	mock.Mock.Test(t)
